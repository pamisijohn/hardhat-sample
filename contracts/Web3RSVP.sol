...
contract Web3RSVP {
    ...
    function confirmAttendee(bytes32 eventId, address attendee) public {
        ...
        // require that msg.sender is owner of the event - only the host should be able to check people in
    ...
    function confirmAllAttendees(bytes32 eventId) external {
        ...
        // Cconfirm each attendee in the rsvp array
        for (uint8 i = 0; i < myEvent.confirmedRSVPs.length; i++){
            confirmAttendee(eventId, myEvent.confirmedRSVPs[i]);
        }
    }

    // SEND UNCLAIMED DEPOSIT TO EVENT ORGANIZER
    function withdrawUnclaimedDeposits(bytes32 eventId) external {
        // look up event
        CreateEvent memory myEvent = idToEvent[eventId];

        // check that the paidOut boolean still equals false AKA the money hasn't already being paid out
